...
const SELECT_LOCATION = state => state.get('routing').get('location').toJS();
...
export function routeReducer(state = initialState, { type, payload: location }) {
  ...
  return state.set('location', fromJS(location));
}
...
export function syncHistory(history) {
  ...
  history.listen(location => { initialState.set('location', location) })();
